      ...
      try {
        switch (ctx.params.action) {
          case 'select':
            ...
            break
          case 'selectWhere':
            result = await ctx.$crud.selectWhere(
          ...
          default:
            ctx.response.status = 400
            result =
              'CRUD操作方式错误，请使用 select, selectWhere, insert, update, del 方法'
        }
        ctx.body = result
      } catch (e) {
    ...
    async init(ctx, next) {
      try {
        ...
        switch (ctx.params.action) {
          ...
          case 'del':
            if (ops?.many) {
              result = await tableDB.deleteMany(where)
            } else {
              result = await tableDB.deleteOne(where)
            }
            break
          case 'update':
            if (ops?.many) {
